...
class Generator:
    ...
    def generate_normal(self, img_id, traits, rec=0):
        ...
        for idx, (layer_id, layer) in enumerate(self.config_normal.items()):
            ...
            attributes.append({'trait_type': layer_id, 'id': trait, 'name': name, 'value': value})
        ...
        self.save_desc(f'{img_id}', metadata)
    ...
    def save_desc(self, name, desc):
        ...
        desc_path = self.build / name
        with open(desc_path, 'w') as desc_file:
            desc_file.write(json.dumps(desc))
